...
* {% for speaker in talk.speakers %}{{ speaker.name }}{{ ", " if not loop.last }} {% endfor %} – `{{ talk.title }} <http://www.writethedocs.org/conf/{{ shortcode }}/{{ year }}/speakers/#speaker-{{ shortcode }}-{{ year }}-{{ talk.speakers.0.slug }}>`__
{% endfor %}
